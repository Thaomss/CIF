...
function FrontOfficeView({ rows, query, setQuery, checked, frontCheckTypes, canEdit, onToggle, onBulkSet, changedCleanIds, cleanFileRef, cleanImporting, onCleanImport, onAcknowledgeClean, onAcknowledgeAllClean, cleanValidating, cleanSetupMissing }: {
  ...
  const [showCleanChanges, setShowCleanChanges] = useState(true)
  const needle = query.trim().toLocaleLowerCase('fr')
  ...
  const changedRows = useMemo(() => sortFrontRows(rows.filter((row) => changedCleanIds.has(row.id)), 'pitch'), [rows, changedCleanIds])
  ...
  function revealCleanChange(id: string) {
    setQuery('')
    window.setTimeout(() => {
      const target = document.getElementById(`front-row-${id}`)
      if (!target) return
      target.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' })
      target.classList.add('focus-clean-change')
      window.setTimeout(() => target.classList.remove('focus-clean-change'), 1900)
    }, 80)
  }
  ...
  return <div className="front-workspace">
    ...
    {changedRows.length > 0 && <section className={`front-changes-menu ${showCleanChanges ? 'is-open' : ''}`}>
      <button type="button" className="front-changes-toggle" aria-expanded={showCleanChanges} onClick={() => setShowCleanChanges((current) => !current)}>
        <span className="front-changes-toggle-icon"><RefreshCw size={16} /></span>
        <span><strong>{changedRows.length} changement{changedRows.length > 1 ? 's' : ''} de nettoyage à valider</strong><small>{showCleanChanges ? 'Chaque changement est affiché ligne par ligne avec son ancien et son nouvel état.' : 'Ouvrez la liste pour voir chaque réservation, l’ancien état et le nouvel état.'}</small></span>
        <ChevronDown size={18} />
      </button>
      {showCleanChanges && <div className="front-changes-list">{changedRows.map((row) => {
        const isReady = row.call_status !== 'cif_pas_possible' && checked(row.id, 'cif_ready')
        return <article className="front-change-line" key={row.id}>
          <div className="front-change-client"><strong>{(row.lastname ?? '').toUpperCase()} {row.firstname}</strong><span><Hash size={12} />{row.reservation_number}<b>{row.pitch || 'Sans emplacement'}</b></span></div>
          <div className="front-change-transition"><span>{cleanLabel(row.clean_previous_status)}</span><ChevronRight size={14} /><strong>{cleanLabel(row.clean_status)}</strong></div>
          <span className={`front-change-list-tag ${isReady ? 'ready' : 'pending'}`}>{isReady ? 'CIF prêt' : 'CIF pas OK'}</span>
          <div className="front-change-actions"><button type="button" onClick={() => revealCleanChange(row.id)}><Eye size={14} />Voir</button>{canEdit && <button type="button" className="validate" onClick={() => void onAcknowledgeClean(row.id)}><Check size={14} />Valider</button>}</div>
        </article>
      })}</div>}
    </section>}
