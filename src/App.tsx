...
import {
  ...
  Tag, Trash2, Upload, Wallet, Wifi, X, AlertTriangle, UsersRound, Printer,
} from 'lucide-react'
...
export default function App() {
  ...
  const [sort, setSort] = useState<'name' | 'unit' | 'status' | 'swikly_todo' | 'swikly_done' | 'travel_todo' | 'travel_done'>('name')
  ...
  useEffect(() => {
    const closeMenus = () => setOpenCallMenu(null)
  ...
  async function deleteReservation(row: Reservation) {
    if (!supabase || !confirm(`Supprimer définitivement la réservation de ${fullName(row) || row.reservation_number} ?${row.is_last_minute ? '' : '\n\nSi elle est encore présente dans un prochain fichier Maxxton, elle sera ajoutée de nouveau lors de la mise à jour.'}`)) return
  ...
  const filtered = useMemo(() => {
    ...
    const result = rows.filter((row) => {
      ...
      return matchesSearch && matchesFilter && matchesCallFilter
    })
    return [...result].sort((a, b) => {
      ...
      if (sort === 'swikly_todo') return Number(checked(a.id, 'swikly')) - Number(checked(b.id, 'swikly')) || fullName(a).localeCompare(fullName(b), 'fr')
      if (sort === 'swikly_done') return Number(checked(b.id, 'swikly')) - Number(checked(a.id, 'swikly')) || fullName(a).localeCompare(fullName(b), 'fr')
      if (sort === 'travel_todo') return Number(checked(a.id, 'travel_party')) - Number(checked(b.id, 'travel_party')) || fullName(a).localeCompare(fullName(b), 'fr')
      if (sort === 'travel_done') return Number(checked(b.id, 'travel_party')) - Number(checked(a.id, 'travel_party')) || fullName(a).localeCompare(fullName(b), 'fr')
      return fullName(a).localeCompare(fullName(b), 'fr')
    })
  }, [rows, checks, checkTypes, query, filter, callFilter, sort])
  ...
  return <div className={`app-shell ${workspace === 'front' ? 'front-shell' : ''}`}>
    ...
    <main className="content">
      ...
      {!day ? <div className="empty"><h2>Aucune journée</h2><p>Créez le prochain samedi d’arrivées pour commencer.</p>{canUseBack && <button className="primary" onClick={() => setShowNewDay(true)}><Plus size={17} />Créer une journée</button>}</div> : workspace === 'front' ?
        <FrontOfficeView rows={rows} query={query} setQuery={setQuery} checked={checked} frontCheckTypes={frontCheckTypes} canEdit={canEditFront} onToggle={toggleCheck} /> :
        <>
          ...
          <section className="toolbar"><div className="search"><Search size={18} /><input value={query} onChange={(e) => { setQuery(e.target.value); setVisibleCount(60) }} placeholder="Rechercher un client, un numéro ou un emplacement…" /></div><div className="toolbar-right"><label className="sort-control"><ArrowUpDown size={15} /><select value={sort} onChange={(e) => setSort(e.target.value as typeof sort)}><option value="name">Nom</option><option value="unit">Emplacement</option><option value="status">CIF à traiter d’abord</option><option value="swikly_todo">Swikly non fait d’abord</option><option value="swikly_done">Swikly fait d’abord</option><option value="travel_todo">Participants non faits d’abord</option><option value="travel_done">Participants faits d’abord</option></select></label><span>{filtered.length} résultat{filtered.length !== 1 ? 's' : ''}</span></div></section>
...
function cleanLabel(status: Reservation['clean_status']) {
  const labels: Record<string, string> = {
    propre: 'Propre', a_controler: 'À contrôler', en_cours: 'En cours', non_propre: 'Non propre', non_renseigne: 'Non renseigné',
  }
...
function FrontOfficeView({ rows, query, setQuery, checked, frontCheckTypes, canEdit, onToggle }: {
  ...
  return <div className="front-workspace">
    <section className="front-hero"><div><span>Suivi accueil en temps réel</span><strong>{ready.length} CIF prêts · {notReady.length} CIF pas OK</strong></div><div className="front-search"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nom, réservation ou emplacement…" /></div></section>
